...
const nichePainPoint = (lead: Lead) => {
  ...
  if (niche.includes("легал") || niche.includes("юрист")) return "у темах документів люди купують довіру до експерта, а не просто послугу";
  if (niche.includes("бухгалтер")) return "клієнту важливо відчути порядок, спокій і контроль у фінансах";
  if (niche.includes("beauty")) return "клієнт обирає людину, стиль, атмосферу і довіру до результату";
  if (niche.includes("авто")) return "людям важливо бачити прозорість процесу і чесність сервісу";
  if (niche.includes("медицин")) return "рішення приймають через довіру до спеціаліста і відчуття безпеки";
  if (niche.includes("освіт")) return "важливо показати не тільки програму, а результат і підхід викладача";
  if (niche.includes("нерух")) return "клієнту потрібні довіра, контекст і пояснення ризиків";
  if (niche.includes("страх")) return "люди хочуть розуміти, що їх не залишать самих у складний момент";
  return "люди швидше довіряють бізнесу, коли бачать людину, процес і реальну цінність";
};

const firstContentIdea = (lead: Lead) => {
  const niche = lead.niche.toLowerCase();
  if (niche.includes("легал") || niche.includes("юрист")) return "показати 2–3 типові помилки клієнтів і ваш підхід до їх вирішення";
  if (niche.includes("бухгалтер")) return "пояснити, як ви наводите порядок у документах і знімаєте стрес із клієнта";
  if (niche.includes("beauty")) return "показати майстра, атмосферу, процес і результат очима клієнта";
  if (niche.includes("авто")) return "показати прозорий процес роботи і що клієнт отримує до оплати";
  if (niche.includes("медицин")) return "показати перший контакт, підхід спеціаліста і чому процес безпечний";
  if (niche.includes("освіт")) return "показати шлях клієнта від проблеми до результату";
  if (niche.includes("нерух")) return "пояснити, як ви допомагаєте клієнту уникнути помилок";
  return "показати людину за бізнесом, підхід і типову проблему клієнта";
};

const hugoWebsiteUrl = "https://hugosite-lac.vercel.app/";
...
const buildPersonalizedMessage = (lead: Lead) => {
  ...
  const weakPoint = lead.weak_point ? `Бачу точку росту: ${lead.weak_point}` : "";
  return [
    `Вітаю${name}! Я Сергій Гальчук / Hugo, незалежний міжнародний журналіст і автор Hugo Media Group.`,
    `Подивився на ${lead.business_name} через ${contentChannelLabel(lead)}. Бачу, що тут можна ${angle}.`,
    weakPoint,
    `У вашій ніші важливо: ${nichePainPoint(lead)}.`,
    `Я можу зробити авторську медійну подачу: коротко показати людину за бізнесом, підсилити довіру і дати контент для соцмереж та комунікації з клієнтами.`,
    `Перший матеріал я б побудував так: ${firstContentIdea(lead)}.`,
    `Про мене і Hugo Media Group: ${hugoWebsiteUrl}`
  ].filter(Boolean).join("\n\n");
};
